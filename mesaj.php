...
$s = testare($_REQUEST["Email"]);
$a = testare($_REQUEST["Subiect"]);
$aba = testare($_REQUEST["Mesaj"]);

if(isset($cnx)) {
$cda= "INSERT into mesaje values('', :n, :e, :s, :m)";
$stmt = $cnx->prepare($cda);
$stmt->bindParam(':n', $n, PDO::PARAM_STR);
$stmt->bindParam(':e', $s, PDO::PARAM_STR);
$stmt->bindParam(':s', $a, PDO::PARAM_STR);
$stmt->bindParam(':m', $aba, PDO::PARAM_STR);
$stmt->execute();

   $cnx = null;
}
?>
    <h2>Vă mulțumim pentru mesaj și vă asigurăm că suntem extrem de preocupați de tot ce putem face în privința aceasta!</h2><br>
          <p></p>
  </div>
