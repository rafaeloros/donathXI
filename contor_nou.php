...
$s = testare($_REQUEST["Scara"]);
$a = testare($_REQUEST["Apartament"]);
$aba = testare($_REQUEST["APA1"]);
$abu = testare($_REQUEST["APA2"]);

if(isset($cnx)) {
$cda= "INSERT into apa values('', :n, :s, :a, :aba, :abu)";
$stmt = $cnx->prepare($cda);
$stmt->bindParam(':n', $n, PDO::PARAM_STR);
$stmt->bindParam(':s', $s, PDO::PARAM_STR);
$stmt->bindParam(':a', $a, PDO::PARAM_STR);
$stmt->bindParam(':aba', $aba, PDO::PARAM_STR);
$stmt->bindParam(':abu', $abu, PDO::PARAM_STR);
$stmt->execute();

   $cnx = null;
}
?>
    <h2>Datele contorului au fost trimise. Vă mulțumim!</h2><br>
          <p></p>
  </div>
